                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h2 class="mb-1 fs-3">
                                ...
                                لوحة متابعة أوامر العمل - جاري العمل
                            </h2>
                            <p class="mb-0 text-white-75">
                                @if(isset($project))
                                    ...
                                    @endif
                                @endif
                                - عرض وتحليل أوامر العمل في حالة جاري التنفيذ
                            </p>
                        </div>
                        <div class="text-end">
                ...
                <div class="col-md-3">
                    <div class="card border-0 bg-info bg-opacity-10">
                        <div class="card-body text-center">
                            ...
                            <h4 class="mb-1 text-info" id="totalInProgressOrders">0</h4>
                            <small class="text-muted">إجمالي أوامر جاري العمل</small>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="card border-0 bg-success bg-opacity-10">
                        <div class="card-body text-center">
                            ...
                            <h4 class="mb-1 text-success" id="totalInProgressValue">0</h4>
            ...
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body p-0">
                    ...
                    <div id="noDataMessage" class="text-center py-5" style="display: none;">
                        ...
                        <h5 class="text-muted">لا توجد أوامر عمل في حالة جاري العمل</h5>
                        <p class="text-muted">جرب تغيير معايير البحث أو التأكد من وجود أوامر عمل في هذه الحالة</p>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('inProgressFilterForm').addEventListener('submit', function(e) {
        e.preventDefault();
        loadInProgressOrders();
    });
    
    // Auto-refresh every 2 minutes
    setInterval(loadInProgressOrders, 120000);
});

let currentPage = 1;
let currentView = 'table';

// Load in-progress work orders
function loadInProgressOrders(page = 1) {
    currentPage = page;
    showLoading();
    
    const formData = new FormData(document.getElementById('inProgressFilterForm'));
    ...
    // Make API call
    fetch(`/api/work-orders/inprogress?${params.toString()}`, {
        headers: {
...
function displayTableResults(data) {
    ...
    data.forEach((order, index) => {
        ...
        row.innerHTML = `
            ...
            <td>
                <strong class="text-success">${formatCurrency(order.order_value_without_consultant)}</strong>
            </td>
            <td>
                <span class="badge bg-warning">جاري العمل</span>
            </td>
            <td>
                <a href="/admin/work-orders/${order.id}" class="btn btn-outline-primary btn-sm" title="عرض التفاصيل">
            ...
            <div class="card work-order-card h-100">
                ...
                <div class="card-body">
                    <div class="row g-2 mb-3">
                        <div class="col-12">
                            <small class="text-muted">المكتب:</small>
                            <div><span class="badge bg-secondary">${order.office || 'غير محدد'}</span></div>
                        </div>
                    </div>
                    
                    <div class="mb-3">
                        <small class="text-muted">القيمة المبدئية (بدون استشاري):</small>
                        <div><strong class="text-success fs-5">${formatCurrency(order.order_value_without_consultant)}</strong></div>
                    </div>
                </div>
                <div class="card-footer bg-transparent">
                    <a href="/admin/work-orders/${order.id}" class="btn btn-outline-primary btn-sm w-100">
...
function updatePagination(pagination) {
    ...
    if (pagination.current_page > 1) {
        paginationHTML += `<li class="page-item"><a class="page-link" href="#" onclick="loadInProgressOrders(${pagination.current_page - 1})">السابق</a></li>`;
    }
    
    // Page numbers
    for (let i = Math.max(1, pagination.current_page - 2); i <= Math.min(pagination.total_pages, pagination.current_page + 2); i++) {
        paginationHTML += `<li class="page-item ${i === pagination.current_page ? 'active' : ''}">
            <a class="page-link" href="#" onclick="loadInProgressOrders(${i})">${i}</a>
        </li>`;
    }
    
    // Next button
    if (pagination.current_page < pagination.total_pages) {
        paginationHTML += `<li class="page-item"><a class="page-link" href="#" onclick="loadInProgressOrders(${pagination.current_page + 1})">التالي</a></li>`;
    }
...
function changeView(view) {
    ...
    if (document.getElementById('resultsTableBody').children.length > 0) {
        ...
        loadInProgressOrders(currentPage);
    }
}

function changePerPage(perPage) {
    loadInProgressOrders(1); // Reset to first page
}

function clearFilters() {
    document.getElementById('inProgressFilterForm').reset();
    loadInProgressOrders(1);
}
