                        ...
                        <div>
                            ...
                            <p class="mb-0 text-white-75">
                                @if(isset($project))
                                    ...
                                    @endif
                                @endif
                                - عرض وتحليل أوامر العمل في المستخلص
                            </p>
                        </div>
                        <div class="text-end">
            ...
            <div class="row mb-4">
                <div class="col-md-4">
                    <div class="card border-0 bg-danger bg-opacity-10">
                        <div class="card-body text-center">
                            ...
                            <h4 class="mb-1 text-danger" id="totalExtractValue">0</h4>
                ...
                <div class="col-md-4">
                    <div class="card border-0 bg-warning bg-opacity-10">
                        <div class="card-body text-center">
                            <i class="fas fa-list-ol text-warning fs-2 mb-2"></i>
                            <h4 class="mb-1 text-warning" id="totalExtractOrders">0</h4>
                ...
                <div class="col-md-4">
                    <div class="card border-0 bg-success bg-opacity-10">
                        <div class="card-body text-center">
                            <i class="fas fa-percentage text-success fs-2 mb-2"></i>
                            <h4 class="mb-1 text-success" id="extractPercentage">0%</h4>
                            <small class="text-muted">نسبة المستخلص</small>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <form id="extractsFilterForm">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                <label for="extract_number" class="form-label">رقم المستخلص</label>
                                <input type="text" class="form-control" id="extract_number" name="extract_number" placeholder="ابحث برقم المستخلص">
                            </div>
            ...
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body p-0">
                    ...
                    <div id="noDataMessage" class="text-center py-5" style="display: none;">
                        ...
                        <h5 class="text-muted">لا توجد أوامر عمل في المستخلص</h5>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('extractsFilterForm').addEventListener('submit', function(e) {
        e.preventDefault();
        loadExtractOrders();
    });
    
    // Auto-refresh every 2 minutes
    setInterval(loadExtractOrders, 120000);
});

let currentPage = 1;
let currentView = 'table';

// Load extract work orders
function loadExtractOrders(page = 1) {
    currentPage = page;
    showLoading();
    
    const formData = new FormData(document.getElementById('extractsFilterForm'));
    ...
    // Make API call
    fetch(`/api/work-orders/extracts?${params.toString()}`, {
        headers: {
...
function displayTableResults(data) {
    ...
    data.forEach((order, index) => {
        ...
        row.innerHTML = `
            ...
            <td>
                <strong class="text-success">${formatCurrency(order.extract_value)}</strong>
            </td>
            <td>
                <strong class="text-danger">${order.extract_number || '-'}</strong>
            </td>
            <td>
                <small class="text-muted">${formatDate(order.extract_date)}</small>
            </td>
            <td>
                <div class="btn-group btn-group-sm">
                    <a href="/admin/work-orders/${order.id}" class="btn btn-outline-danger" title="عرض التفاصيل">
    ...
    data.forEach(order => {
        ...
        card.innerHTML = `
                    <div class="card work-order-card h-100">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                <small class="text-muted">رقم المستخلص:</small>
                                <div><strong class="text-danger">${order.extract_number || '-'}</strong></div>
                            </div>
                            
                            <div class="mb-3">
                                <small class="text-muted">تاريخ المستخلص:</small>
                                <div><small class="text-muted">${formatDate(order.extract_date)}</small></div>
                            </div>
                            ...
                            <div class="mb-3">
                                <small class="text-muted">قيمة المستخلص:</small>
                                <div><strong class="text-success fs-5">${formatCurrency(order.extract_value)}</strong></div>
                            </div>
                        </div>
                        <div class="card-footer bg-transparent">
                            <div class="btn-group w-100">
                                <a href="/admin/work-orders/${order.id}" class="btn btn-outline-danger btn-sm">
...
function updateSummaryCards(summary) {
    document.getElementById('totalExtractValue').textContent = formatCurrency(summary.total_extract_value || 0);
    document.getElementById('totalExtractOrders').textContent = summary.total_orders || 0;
    document.getElementById('extractPercentage').textContent = (summary.extract_percentage || 0) + '%';
...
function updatePagination(pagination) {
    ...
    if (pagination.current_page > 1) {
        paginationHTML += `<li class="page-item"><a class="page-link" href="#" onclick="loadExtractOrders(${pagination.current_page - 1})">السابق</a></li>`;
    }
    
    // Page numbers
    for (let i = Math.max(1, pagination.current_page - 2); i <= Math.min(pagination.total_pages, pagination.current_page + 2); i++) {
        paginationHTML += `<li class="page-item ${i === pagination.current_page ? 'active' : ''}">
            <a class="page-link" href="#" onclick="loadExtractOrders(${i})">${i}</a>
        </li>`;
    }
    
    // Next button
    if (pagination.current_page < pagination.total_pages) {
        paginationHTML += `<li class="page-item"><a class="page-link" href="#" onclick="loadExtractOrders(${pagination.current_page + 1})">التالي</a></li>`;
    }
...
function changeView(view) {
    ...
    if (document.getElementById('resultsTableBody').children.length > 0) {
        ...
        loadExtractOrders(currentPage);
    }
}

function changePerPage(perPage) {
    loadExtractOrders(1); // Reset to first page
}

function clearFilters() {
    document.getElementById('extractsFilterForm').reset();
    loadExtractOrders(1);
}
...
function exportResults() {
    ...
    window.open(`/api/work-orders/extracts/export?${params.toString()}`, '_blank');
}
