...
<div class="container py-3">
    <div class="row justify-content-center">
        <div class="col-md-10">
            ...
            <!-- جدول بيانات الرخص -->
            <div class="card license-table mt-3">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover">
                            <thead class="table-light">
                                <tr>
                                    <th class="text-center" colspan="2">معلومات الرخصة</th>
                                </tr>
                            </thead>
                            <tbody>
                                <!-- شهادات التنسيق -->
                                <tr>
                                    <td class="fw-bold" style="width: 30%">شهادات التنسيق</td>
                                    <td>
                                        @if($workOrder->licenses->first()?->coordination_certificate_path)
                                            <a href="{{ asset('storage/' . $workOrder->licenses->first()->coordination_certificate_path) }}" target="_blank" class="btn btn-sm btn-info">
                                                <i class="fas fa-eye"></i> عرض الشهادة
                                            </a>
                                        @else
                                ...
                                <!-- تفعيل الرخصة -->
                                <tr>
                                    <td class="fw-bold">تفعيل الرخصة</td>
                                    <td>
                                        <div class="row">
                                            <div class="col-md-4">
                                                <p class="mb-2"><strong>الرخصة الأولى:</strong></p>
                                                @if($workOrder->licenses->first()?->license_1_path)
                                                    <a href="{{ asset('storage/' . $workOrder->licenses->first()->license_1_path) }}" target="_blank" class="btn btn-sm btn-info">
                                                        <i class="fas fa-eye"></i> عرض المرفق
                                                    </a>
                                                @else
                                                    <span class="text-muted">لا يوجد</span>
                                            ...
                                            <div class="col-md-4">
                                                <p class="mb-2"><strong>طول الرخصة:</strong></p>
                                                @if($workOrder->licenses->first()?->license_length)
                                                    {{ $workOrder->licenses->first()->license_length }} متر
                                                @else
                                                    <span class="text-muted">غير محدد</span>
                                                @endif
                                            </div>
                                            <div class="col-md-4">
                                                <p class="mb-0"><strong>أيام التنبيه قبل الانتهاء:</strong> {{ $workOrder->licenses->first()?->license_alert_days ?? 30 }} يوم</p>
                                            </div>
                                        </div>
                                        @if($workOrder->licenses->first()?->license_start_date && $workOrder->licenses->first()?->license_end_date)
                                            <div class="mt-3">
                                                <p class="mb-1"><strong>تاريخ بداية الرخصة:</strong> {{ $workOrder->licenses->first()->license_start_date }}</p>
                                                <p class="mb-1"><strong>تاريخ نهاية الرخصة:</strong> {{ $workOrder->licenses->first()->license_end_date }}</p>
                                            </div>
                                        @endif
                                    </td>
                                </tr>
                                ...
                                <!-- الحظر -->
                                <tr>
                                    <td class="fw-bold">الحظر</td>
                                    <td>
                                        @if($workOrder->licenses->first()?->has_restriction)
                                            نعم - {{ $workOrder->licenses->first()->restriction_authority }}
                                        @else
                                            لا يوجد حظر
                                        @endif
                                    </td>
                                ...
                                <!-- فواتير السداد -->
                                <tr>
                                    <td class="fw-bold">فواتير السداد</td>
                                    <td>
                                        @if($workOrder->licenses->first()?->payment_invoices_path)
                                            <a href="{{ asset('storage/' . $workOrder->licenses->first()->payment_invoices_path) }}" target="_blank" class="btn btn-sm btn-info">
                                                <i class="fas fa-eye"></i> عرض المرفق
                                            </a>
                                        @else
                                            <span class="text-muted">لا يوجد</span>
                                ...
                                <!-- إثبات السداد -->
                                <tr>
                                    <td class="fw-bold">إثبات السداد</td>
                                    <td>
                                        @if($workOrder->licenses->first()?->payment_proof_path)
                                            <a href="{{ asset('storage/' . $workOrder->licenses->first()->payment_proof_path) }}" target="_blank" class="btn btn-sm btn-info">
                                                <i class="fas fa-eye"></i> عرض المرفق
                                            </a>
                                        @else
                                            <span class="text-muted">لا يوجد</span>
                                ...
                                <!-- الاختبارات -->
                                <tr>
                                    <td class="fw-bold">الاختبارات</td>
                                    <td>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <p><strong>اختبار العمق:</strong> 
                                                    @if($workOrder->licenses->first()?->has_depth_test)
                                                        تم بتاريخ {{ $workOrder->licenses->first()->depth_test_date }}
                                                    @else
                                                        لم يتم
                                                    @endif
                                                </p>
                                                <p><strong>اختبار دك التربة:</strong>
                                                    @if($workOrder->licenses->first()?->has_soil_compaction_test)
                                                        تم بتاريخ {{ $workOrder->licenses->first()->soil_compaction_test_date }}
                                                    @else
                                                        لم يتم
                                                    @endif
                                                </p>
                                                <p><strong>اختبار RC1-MC1:</strong>
                                                    @if($workOrder->licenses->first()?->has_rc1_mc1_test)
                                                        تم بتاريخ {{ $workOrder->licenses->first()->rc1_mc1_test_date }}
                                                    @else
                                                        لم يتم
                                                    @endif
                                                </p>
                                            ...
                                            <div class="col-md-6">
                                                <p><strong>اختبار الأسفلت:</strong>
                                                    @if($workOrder->licenses->first()?->has_asphalt_test)
                                                        تم بتاريخ {{ $workOrder->licenses->first()->asphalt_test_date }}
                                                    @else
                                                        لم يتم
                                                    @endif
                                                </p>
                                                <p><strong>اختبار التربة:</strong>
                                                    @if($workOrder->licenses->first()?->has_soil_test)
                                                        تم بتاريخ {{ $workOrder->licenses->first()->soil_test_date }}
                                                    @else
                                                        لم يتم
                                                    @endif
                                                </p>
                                                <p><strong>اختبار البلاط والانترلوك:</strong>
                                                    @if($workOrder->licenses->first()?->has_interlock_test)
                                                        تم بتاريخ {{ $workOrder->licenses->first()->interlock_test_date }}
                                                    @else
                                                        لم يتم
                                                    @endif
                                                </p>
                                ...
                                <!-- تمديد الرخصة -->
                                <tr>
                                    <td class="fw-bold">تمديد الرخصة</td>
                                    <td>
                                        @if($workOrder->licenses->first()?->license_extension_file_path)
                                            <div class="mb-2">
                                                <a href="{{ asset('storage/' . $workOrder->licenses->first()->license_extension_file_path) }}" target="_blank" class="btn btn-sm btn-info">
                                                    <i class="fas fa-eye"></i> عرض المرفق
                                                </a>
                                            </div>
                                            <p class="mb-1">تاريخ بداية التمديد: {{ $workOrder->licenses->first()->license_extension_start_date }}</p>
                                            <p class="mb-0">تاريخ نهاية التمديد: {{ $workOrder->licenses->first()->license_extension_end_date }}</p>
                                        @else
                                ...
                                <!-- تمديد الفاتورة -->
                                <tr>
                                    <td class="fw-bold">تمديد الفاتورة</td>
                                    <td>
                                        @if($workOrder->licenses->first()?->invoice_extension_file_path)
                                            <div class="mb-2">
                                                <a href="{{ asset('storage/' . $workOrder->licenses->first()->invoice_extension_file_path) }}" target="_blank" class="btn btn-sm btn-info">
                                                    <i class="fas fa-eye"></i> عرض المرفق
                                                </a>
                                            </div>
                                            <p class="mb-1">تاريخ بداية التمديد: {{ $workOrder->licenses->first()->invoice_extension_start_date }}</p>
                                            <p class="mb-0">تاريخ نهاية التمديد: {{ $workOrder->licenses->first()->invoice_extension_end_date }}</p>
                                        @else
                                            <span class="text-muted">لا يوجد تمديد</span>
                                        @endif
                                    </td>
                                ...
                                <!-- نتائج الاختبار -->
                                <tr>
                                    <td class="fw-bold">نتائج الاختبار</td>
                                    <td>
                                        @if($workOrder->licenses->first()?->test_results_file_path)
                                            <a href="{{ asset('storage/' . $workOrder->licenses->first()->test_results_file_path) }}" target="_blank" class="btn btn-sm btn-info">
                                                <i class="fas fa-eye"></i> عرض المرفق
                                            </a>
                                        @else
                                            <span class="text-muted">لا يوجد</span>
                                ...
                                <!-- الفحص النهائي -->
                                <tr>
                                    <td class="fw-bold">الفحص النهائي</td>
                                    <td>
                                        @if($workOrder->licenses->first()?->final_inspection_file_path)
                                            <a href="{{ asset('storage/' . $workOrder->licenses->first()->final_inspection_file_path) }}" target="_blank" class="btn btn-sm btn-info">
                                                <i class="fas fa-eye"></i> عرض المرفق
                                            </a>
                                        @else
                                            <span class="text-muted">لا يوجد</span>
                                ...
                                <!-- إخلاء وإغلاق الرخصة -->
                                <tr>
                                    <td class="fw-bold">إخلاء وإغلاق الرخصة</td>
                                    <td>
                                        @if($workOrder->licenses->first()?->license_closure_file_path)
                                            <a href="{{ asset('storage/' . $workOrder->licenses->first()->license_closure_file_path) }}" target="_blank" class="btn btn-sm btn-info">
                                                <i class="fas fa-eye"></i> عرض المرفق
                                            </a>
                                        @else
                                            <span class="text-muted">لا يوجد</span>
