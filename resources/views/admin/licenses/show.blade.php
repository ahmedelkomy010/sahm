            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="border rounded p-3">
                    ...
                    @if($license->license_file_path)
                        <a href="{{ asset($license->license_file_path) }}" target="_blank" class="text-blue-500 hover:underline">
                            عرض الملف
                        </a>
                    @else
                ...
                <div class="border rounded p-3">
                    <h4 class="font-medium mb-2">ملفات التفعيل</h4>
                    @if($license->activation_files && count($license->activation_files) > 0)
                        @foreach($license->activation_files as $file)
                            <a href="{{ asset($file) }}" target="_blank" class="block text-blue-500 hover:underline mb-1">
                                عرض الملف {{ $loop->iteration }}
                            </a>
                        @endforeach
                    @else
                        <span class="text-gray-500">لا توجد ملفات</span>
                    @endif
                </div>

                <div class="border rounded p-3">
                    <h4 class="font-medium mb-2">فواتير السداد</h4>
                    @if($license->payment_receipts && count($license->payment_receipts) > 0)
                        @foreach($license->payment_receipts as $receipt)
                            <a href="{{ asset($receipt) }}" target="_blank" class="block text-blue-500 hover:underline mb-1">
                                عرض الفاتورة {{ $loop->iteration }}
                            </a>
                        @endforeach
                    @else
                        <span class="text-gray-500">لا توجد فواتير</span>
                    @endif
                </div>

                <div class="border rounded p-3">
                    <h4 class="font-medium mb-2">إثبات السداد</h4>
                    @if($license->payment_proof_files && count($license->payment_proof_files) > 0)
                        @foreach($license->payment_proof_files as $proof)
                            <a href="{{ asset($proof) }}" target="_blank" class="block text-blue-500 hover:underline mb-1">
                                عرض الإثبات {{ $loop->iteration }}
                            </a>
                        @endforeach
                    @else
                        <span class="text-gray-500">لا يوجد إثبات</span>
                    @endif
                </div>
            </div>
        </div>
