...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-yellow-50 to-amber-100 py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="header-gradient rounded-3xl shadow-2xl overflow-hidden mb-12 relative">
            <div class="relative z-10 p-8 text-center text-white">
                <div class="mb-6">
                    ...
                    <h1 class="text-4xl font-bold mb-3">Base Design</h1>
                    <p class="text-yellow-100 text-lg">{{ $project->name }}</p>
                    <p class="text-yellow-100 text-sm mt-2">رقم العقد: {{ $project->contract_number }}</p>
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                        <p class="text-yellow-100 text-sm mb-1">نوع المشروع</p>
                        <p class="font-semibold text-lg">{{ $project->getProjectTypeText() }}</p>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                        <p class="text-yellow-100 text-sm mb-1">الموقع</p>
        ...
        <!-- Create Folder & Upload Base Design -->
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-8">
            <div class="p-8">
                <h2 class="text-xl font-bold text-yellow-900 mb-6 text-right">إدارة المجلدات والمرفقات</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div class="bg-gradient-to-br from-yellow-50 to-white rounded-xl p-6 border-2 border-yellow-100">
                        <div class="flex items-center mb-4">
                            ...
                            <h3 class="text-lg font-semibold text-yellow-900">إنشاء مجلد جديد</h3>
                        </div>
                        
                        <form action="{{ route('projects.design.base.create-folder', $project) }}" method="POST" class="space-y-4">
                            @csrf
                            <div>
                                <label for="folder_name" class="block text-sm font-medium text-yellow-700 mb-2 text-right">اسم المجلد</label>
                                <input type="text" 
                                       id="folder_name" 
                                       name="folder_name" 
                                       required
                                       class="w-full px-4 py-3 border border-yellow-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 text-right"
                                       placeholder="مثال: مخططات القواعد">
                                @error('folder_name')
                            ...
                            <div>
                                <label for="folder_description" class="block text-sm font-medium text-yellow-700 mb-2 text-right">الوصف (اختياري)</label>
                    ...
                    <div class="bg-gradient-to-br from-yellow-50 to-white rounded-xl p-6 border-2 border-yellow-100">
                        <div class="flex items-center mb-4">
                            ...
                            <h3 class="text-lg font-semibold text-yellow-900">رفع ملفات متعددة</h3>
                        </div>
                        
                        <form action="{{ route('projects.design.base.upload-files', $project) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                            @csrf
                            <div>
                                <label for="folder_id" class="block text-sm font-medium text-yellow-700 mb-2 text-right">اختر المجلد</label>
                                <select id="folder_id" 
                                        name="folder_id" 
                                        class="w-full px-4 py-3 border border-yellow-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 text-right">
                            ...
                            <div>
                                <label for="files" class="block text-sm font-medium text-yellow-700 mb-2 text-right">
                                    اختر الملفات
                                    <span class="text-xs text-yellow-500">(يمكنك اختيار ملفات متعددة)</span>
                                </label>
                                <div class="relative">
                                    <input type="file" 
                                           id="files" 
                                    ...
                                    <label for="files" 
                                           class="flex items-center justify-center w-full px-4 py-8 border-2 border-dashed border-yellow-300 rounded-lg cursor-pointer hover:border-yellow-500 hover:bg-yellow-50 transition-all">
                                        <div class="text-center">
                                            <svg class="w-10 h-10 text-yellow-400 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path>
                                            </svg>
                                            <p class="text-sm text-yellow-600">اضغط لاختيار الملفات</p>
                                            <p class="text-xs text-yellow-500 mt-1">أو اسحب الملفات هنا</p>
                                        </div>
                                    </label>
                    </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="p-8">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-xl font-bold text-yellow-900">المجلدات والملفات</h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="bg-gradient-to-br from-yellow-50 to-white border-2 border-yellow-100 rounded-xl p-4 hover:shadow-lg transition-all duration-300 hover:scale-105 cursor-pointer">
                            <div class="flex items-start mb-3">
                                <div class="w-12 h-12 bg-yellow-100 rounded-lg flex items-center justify-center ml-3 flex-shrink-0">
                                    <svg class="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path>
                                    </svg>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <h3 class="text-lg font-semibold text-yellow-900 mb-1 truncate text-right">{{ $folder['name'] }}</h3>
                                    @if($folder['description'])
                                    <p class="text-xs text-yellow-500 line-clamp-2 text-right">{{ $folder['description'] }}</p>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="flex items-center justify-between text-sm pt-3 border-t border-yellow-100">
                                ...
                                <span class="text-yellow-500">{{ $folder['file_count'] }} ملفات</span>
                            </div>
                            
                            <div class="flex items-center gap-2 mt-3">
                                <a href="{{ route('projects.design.base.folder', ['project' => $project, 'folderName' => $folder['name']]) }}" class="flex-1 px-3 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg text-sm transition-colors text-center">
                        ...
                        <div class="bg-gradient-to-br from-yellow-50 to-white border-2 border-yellow-100 rounded-xl p-4 hover:shadow-lg transition-all duration-300 hover:scale-105">
                            <div class="flex items-start mb-3">
                                <div class="w-12 h-12 bg-yellow-100 rounded-lg flex items-center justify-center ml-3 flex-shrink-0">
                                    <svg class="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h3 class="text-sm font-semibold text-yellow-900 mb-1 truncate text-right">{{ $file['name'] }}</h3>
                                    <p class="text-xs text-yellow-500">{{ number_format($file['size'] / 1024, 2) }} KB</p>
                                </div>
                            </div>
                            
                            <div class="flex items-center justify-between text-xs pt-3 border-t border-yellow-100">
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <h3 class="text-lg font-semibold text-yellow-900 mb-2">لا توجد مجلدات أو ملفات</h3>
                        <p class="text-yellow-600">ابدأ بإنشاء مجلد جديد أو رفع ملفات</p>
                    </div>
                @endif
            </div>
        </div>
...
<div id="renameModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-lg bg-white">
        <div class="mt-3">
            ...
            <form id="renameForm" action="{{ route('projects.design.base.rename-folder', $project) }}" method="POST">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <input type="text" 
                           ...
                           name="new_name" 
                           required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 text-right"
...
function updateFileList(input) {
    ...
    if (input.files.length > 0) {
        Array.from(input.files).forEach((file, index) => {
            ...
            fileItem.innerHTML = `
                <div class="flex items-center flex-1">
                    ...
                    <div class="flex-1">
                        <p class="text-sm font-medium text-yellow-900 text-right">${file.name}</p>
                        <p class="text-xs text-yellow-500 text-right">${formatFileSize(file.size)}</p>
                    </div>
                </div>
        ...
        summary.className = 'mt-3 p-3 bg-yellow-50 rounded-lg border border-yellow-200';
        summary.innerHTML = `
            <p class="text-sm text-yellow-700 font-medium text-right">
